...
class OCString {
  ...
  public:
    ...
    // length characters.
    OCString (const char* str, size_type len) 
    {
      build_(str, len);
    }
    ...
    // Copy constructor.     
    OCString (const OCString& str) 
    {
      build_(str.data(), str.length());
    }
  ...
  private:
    ...
    void build_ (const char *str, size_type len) {
      if (len > OC_MAX_INTERNAL_LENGTH - 2) {
	u_.s_.ptr_= new char[len + 1];
...
#else
#include <stdexcept>
inline void OCThrowRangeEx (const char* routine, unsigned int i,unsigned high){ throw std::out_of_range(routine); }
#endif
// ///////////////////////////////////////////// Global Methods
...
{
  const OCString::size_type len1 = s1.length();
  const OCString::size_type len2 = s2.length();
  if (len1==len2) {
    if (len1==0)
      return 1;
    else
