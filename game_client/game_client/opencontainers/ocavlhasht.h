...
template <class K, class V, int_u4 CHUNKSIZE> class AVLHashTIterator2;
...
class AVLHashT2 {
    ...
    friend class AVLHashTIterator2<K,V,CHUNKSIZE>;
  ...
  public:
    ...
    // Copy Constructor
    AVLHashT2 (const AVLHashT2<K,V,CHUNKSIZE>& rhs) :
      entries_(0)
    { 
      helpConstruct_();
      copyTable_(rhs);
    ...
    // Assignment
    AVLHashT2<K,V,CHUNKSIZE>& operator= (const AVLHashT2<K,V,CHUNKSIZE>& rhs)
    {
      clear();
      copyTable_(rhs);
      return *this;
    ...
    // functions)
    bool operator== (const AVLHashT2<K,V,CHUNKSIZE>& t1) const
    {
	...
	return notInTableInsert_(node, key, hashkey, V(), found_where);
      }
    }
    ...
    void swap (AVLHashT2<K,V,CHUNKSIZE>& rhs)
    {
      ::swap(root_,     rhs.root_);
      ::swap(freelist_, rhs.freelist_);
  ...
  protected:
    ...
    V& notInTableInsert_ (N* parent, const K& key, int_u4 keyhash, 
	...
	current->hashkey = inorder_succ->hashkey;
	current->value = inorder_succ->value;
	// The node moving up may have a bucketlist!
	if (inorder_succ->next) {
	  current->next  = inorder_succ->next;
	  inorder_succ->next->parent_ = current; // DOUBLY-linked list
	}
	...
	front = node + node->on_freelist_ref_count;
      ...
      return new (node) N(left, right, parent, key, keyhash, value); 
    }
    ...
    void copyTable_ (const AVLHashT2<K,V,CHUNKSIZE>& rhs) 
    {
      // Iterate and insert
    ...
    bool threadedOK_ () 
    {
      OCArray< N* > in_rec, in_for, in_back;
	...
	return false;
      ...
      for (int ii=0; ii< in_rec.length(); ii++) {
	if (in_rec[ii] != in_for[ii]) 
	  return false;
	if (in_rec[ii] != in_back[in_rec.length()-ii-1])
	  return false;
      }
    ...
    void inorderRecursive_ (N* node,  OCArray< N* >& a)
    {
      if (node==0) return;
...
class AVLHashT {
  ...
  public:
    ...
    // Copy Constructor
    AVLHashT (const AVLHashT<K,V,CHUNKSIZE>& rhs) :
      entries_(0)
    { 
      helpConstruct_();
      copyTable_(rhs);
...
class AVLHashTIterator2 {
  ...
  public:
    ...
    // Constructor
    AVLHashTIterator2 (const AVLHashT2<K,V,CHUNKSIZE>& tree) :
      tree_(&tree) { reset(); }
    ...
    // Resets the iterator to iterate over collection c
    void reset (const AVLHashT2<K,V,CHUNKSIZE>& c) { tree_=&c; reset(); }
    ...
    // Backpointer to container
    AVLHashT2<K, V, CHUNKSIZE>* container () const 
    { typedef AVLHashT2<K, V, CHUNKSIZE> NCHT; return (NCHT*)tree_; }
  ...
  protected:
    ...
    // The AVL tree we are looking at.  Not adopted, just reffed.
    const AVLHashT2<K, V, CHUNKSIZE>* tree_;
...
template <class K, class V> struct AVLNR {
  AVLNR (AVLNode_<K, V>* data) : node(data) { }
  void swap (AVLNR<K,V>& rhs) { ::swap(node, rhs.node); }
...
class AVLHashTSortedIterator {
  public:
    ...
    bool next () { return (unsigned)++current_ < keys_.length(); }
    bool operator() () { return next(); }
